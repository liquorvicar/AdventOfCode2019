...
class Answer07Test extends BaseTest
{
     ...
     */
    public function testOne($program, $maxSignal)
    {
        $answer = new Answer07($this->logger);
        $this->assertEquals($maxSignal, $answer->one($program));
    }
     ...
     */
    public function testTwo($program, $maxSignal)
    {
        $answer = new Answer07($this->logger);
        $this->assertEquals($maxSignal, $answer->two($program));
    }
    ...
    public function testMaxSignal()
    {
        ...
        $answer = new Answer07($this->logger);
        $signal = $answer->runLoopedAmplifier([], $computers, [], $program);
        $this->assertEquals(18216, $signal);
    }
}